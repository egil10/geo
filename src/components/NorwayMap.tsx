...
export default function NorwayMap({ region, pin }: { region?: string; pin?: { x: number; y: number } }) {
  return (
    <svg viewBox={GEO_VIEWBOX} className="h-full w-full" preserveAspectRatio="xMidYMid meet" role="img" aria-label="Kart over Norge">
      ...
      {pin && (
        <g>
          <circle cx={pin.x} cy={pin.y} r="17" fill="#e11d48" opacity="0.25" />
          <circle cx={pin.x} cy={pin.y} r="9" fill="#e11d48" stroke="#fff" strokeWidth="3" />
        </g>
      )}
    </svg>
  );
